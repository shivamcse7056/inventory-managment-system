...
const AddModal = ({ isOpen, onClose, type, categories = [], onSuccess, showNotification }) => {
    ...
    try {
      if (type === 'product') {
        ...
        Object.keys(productData).forEach(key => {
          postData.append(key, productData[key]);
        });
        if (imageFile) {
          postData.append('image', imageFile);
        }

        const res = await createProductApi(postData);
        if (res.success) {
      ...
      showNotification(err.message || 'Failed to create item', false);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-2xl overflow-y-auto max-h-[90vh] animate-scaleIn">
        <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800">
          <h3 className="text-lg font-bold text-slate-800 dark:text-white">
            {type === 'product' ? 'Add New Product' : 'Add New Category'}
          </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4 text-left">
          {type === 'product' ? (
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Initial Quantity</label>
                  <input
                    type="number"
                    name="quantity"
                    min="0"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Product Image</label>
                <input 
                  type="file" 
                  accept="image/*" 
          ...
          <div className="flex justify-end space-x-2 pt-4 border-t border-slate-100 dark:border-slate-800">
            ...
            >
              {loading ? (
                ...
                type === 'product' ? 'Create Product' : 'Create Category'
              )}
            </button>
          </div>
        </form>
